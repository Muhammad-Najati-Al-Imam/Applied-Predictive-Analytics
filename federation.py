...
Config_gaussian_noise_scale = 0.1 #?
Config_n_epochs = 50
...
class Federated:
    def __init__(self, split_training_datasets, split_testing_datasets, model_type, noise):
        ...
        for split in range(1): #number of folds
    ...
    def gaussian_noise(self, model_params):
        ...
        for param, tensor in model_params.items():
            ...
            noise_tensor *= Config_gaussian_noise_scale
    ...
    def test_global_model(self, model, epoch):
        ...
        for facility, df_test in self.training_datasets.items():
            X_test = torch.Tensor(
                df_test.drop('converted', axis=1).values.astype('float32')).to(self.device)
            y_test = torch.Tensor(
                df_test[['converted']].values.astype('float32')).to(self.device)
            ...
            auroc = auc(fpr, tpr)
            print('Dataset:', facility, '/',  'AUC:' , auroc)
    ...
    def gaping_maw(self):
        ...
        for epoch in range(Config_n_epochs):
            print('Epoch:', epoch)
        ...
        save_path = f'{output_folder}/{self.model_type}_Federated_{self.current_split}.pth'
        if self.noise:
            save_path = f'{output_folder}/{self.model_type}_Federated_{self.current_split}_noisy.pth'
        ...
        pd.to_pickle(
            self.dict_metrics_split,
            f'{output_folder}/ModelEvolution_{self.model_type}_Noise_{self.noise}_seed_{self.seed}.pickle', protocol=4)
...
def global_pooling(training_datasets, testing_datasets):
    ...
    print('Training Pooled data')

    facilities = list(training_datasets.keys())
    cols = training_datasets[1][0].columns
    ...
    for split in range(1):
        ...
        if df_train.columns[-1] != 'converted':
            print('Nope.')
            exit(1)
        ...
        training_dataloader = torch.utils.data.DataLoader(
            ...
            batch_size=Config_batch_size, shuffle=True, num_workers=4)
        ...
        for _ in range(Config_n_epochs):
            model.train()
        ...
        torch.save(best_state_dict, f'{output_folder}/MLP_Pooled_{split}.pth')
        torch.save(lso.state_dict(), f'{output_folder}/LASSO_Pooled_{split}.pth')


def model_comparisons(training_datasets, testing_datasets, mode, splits=0):
    torch.manual_seed(seed)
    ...
    for split in range(1):
        ...
        for facility in hospitals:
            ...
            x_test = torch.Tensor(df_test.drop('converted', axis=1).values.astype('float32'))
            y_test = df_test[['converted']].values.astype('float32')

            # Patient proportions
            total_train = len(df_train)
            train_perc = df_train['converted'].sum() / len(df_train)
            test_perc = df_test['converted'].sum() / len(df_test)

            # Sanity check
            if df_train.columns[-1] != df_test.columns[-1] != 'converted':
                print('Yeah, no.')
                exit(1)
            ...
            pooled_LASSO_state_dict_path = f'{output_folder}/LASSO_Pooled_{split}.pth'
            ...
            federated_LASSO_state_dict_path = f'{output_folder}/LASSO_Federated_{split}.pth'
            ...
            federated_MLP_state_dict_path = f'{output_folder}/MLP_Federated_{split}.pth'
            ...
            pooled_MLP_state_dict_path = f'{output_folder}/MLP_Pooled_{split}.pth'
            ...
            pred_dict = {
                'MLP: Local': mlp_pred,
                'MLP: Federated': global_pred,
                'MLP: Global': global_pred_pooled,
                'LASSO: Local': lso_pred,
                'LASSO: Federated': global_lso_pred,
                'LASSO: Global': lso_pooled_pred
            }
    ...
    pd.to_pickle(
        dict_metrics,
        f'{output_folder}/ModelComparisons_{mode}.pickle',
        protocol=4)
    ...
    df_results.to_pickle(
        f'{output_folder}/ModelComparisonsTabulated_{mode}.pickle',
        protocol=4)
    
    df_results = df_results.reset_index()
    df_results = df_results.rename(columns={'FACILITY': 'STORE'})
    df_results = df_results.groupby('MODEL').agg({'AUROC' : ['mean', 'min', 'max']}).reset_index()
    df_results.columns = [' '.join(col).strip() for col in df_results.columns.values]

    df_results['SPLIT'] = splits
...
# =============================================================================
def get_model_comparison(training_datasets, testing_datasets, mode, splits=0):
    Federated(split_training_datasets=training_datasets, split_testing_datasets=testing_datasets, model_type='MLP', noise=None)
    Federated(split_training_datasets=training_datasets, split_testing_datasets=testing_datasets, model_type='LASSO', noise=None)
    global_pooling(training_datasets, testing_datasets)
    results = model_comparisons(training_datasets, testing_datasets, mode=mode, splits=splits)
    return results